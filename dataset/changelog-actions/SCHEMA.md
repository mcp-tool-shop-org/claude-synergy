...
- **`output.subject` ≤ 200 chars.** Single line, no markdown formatting beyond inline backticks for surface names. Validator: `scripts/validate-dataset.mjs` rule `subject`.
- **`output.action_text` ≤ 500 chars (hard cap).** Style spec target is 30–80 words / 150–400 chars; validator enforces only the hard cap, not the target band. See [STYLE.md](./STYLE.md) for the full voice/length spec. Validator: rule `action_text.length`.
- **`output.tags` is an array of up to 10 lowercase-kebab strings.** Each tag matches `^[a-z0-9][a-z0-9-]*$` — alphanumeric + hyphens, no dots, no underscores, no uppercase. (Period in tags like `opus-4.6` is NOT allowed — use `opus-4-6`.) Validator: rules `tags.count` and `tags.pattern`.
- **`input.change_id` is shaped as `<namespace>/<...>/<ordinal>`.** Namespace is either a product slug matching `^[a-z0-9][a-z0-9-]*$` (corpus-bound) or one of the external-namespace allowlist (`external-ghsa`, `external-cve`). Ordinal (last segment) must be a positive integer. Middle segment(s) carry the version string; scoped npm versions like `@modelcontextprotocol/client@2.0.0-alpha.1` therefore introduce additional `/` separators inside the change_id, which is allowed. Validator: rules `change_id.namespace` and `change_id.ordinal`.
...
- **`provenance.review_action` must be `"approved"` or `"edited"` for an entry to land in `training.jsonl`.** Both states represent a human having vetted the entry — `approved` means accepted as-labeled; `edited` means accepted after human modification. The build script accepts either in strict mode; in `--include-pending` dev mode it also accepts `pending`. `"rejected"` entries stay in `entries/` as negative-space documentation but are excluded from training under any mode.
...
For backpropagate's `Trainer.train()`. Qwen2.5 chat format. The system prompt is part of the dataset contract — changing it requires a new fine-tune. The example below is the actual prompt the build script emits; the canonical source-of-truth lives in `scripts/build-dataset.mjs` (`SYSTEM_PROMPT` constant) and is also captured in `manifest.json` per build so historical fine-tunes can re-derive their prompt.

```jsonl
{"messages": [
  {"role": "system", "content": "You synthesize one changelog entry into one structured action item. Output strict JSON with fields: kind (one of: breaking | deprecation | security | feature | fix | performance | docs | unknown), severity (one of: low | medium | high), subject, action_text (verb-first imperative, 1-3 sentences, names the affected surface in backticks, max 500 chars), deadline (YYYY-MM-DD if upstream-named, else null), tags. Use kind='unknown' when you cannot classify confidently. Never invent deadlines."},
...
The derived JSONL is regenerated by `scripts/build-dataset.mjs` from approved/edited entries in `entries/`. Don't hand-edit.
